                ...
                for row_index, table_row in enumerate(table.find_all("tr")):
                    ...
                    if link:
                        try:
                            ...
                            schedule_url = url_regex.search(schedule_url).group(1)  + "schedule"
                            schedule_response = requests.get(schedule_url)
                            if schedule_response.ok:
                                soup2 = BeautifulSoup(schedule_response.content, "html.parser")
                                secondary_link = soup2.find("a", string="Print")
                                if secondary_link:
                                    desired_url = secondary_link.get("href")
                                else:
                                    print(f"Error finding the print button on the webpage {schedule_response.url}? skipping...")
                                    continue
                                if desired_url:
                                    writer.write(desired_url + "\n")
                                else:
                                    print(f"Error extracting URL from {link.get_text()}")
                            else:
                                print(f"Error getting the schedule website for {link.get_text()}: {schedule_url}")
                        except ConnectionError:
                            print(f"Website would not connect, skipping...: {link.get('href')}")
                            continue
                    else:
                        print(f"Error getting website link off of table. (row {row_index})")
        else:
